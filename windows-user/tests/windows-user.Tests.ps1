...
Describe 'windows-user' {
    ...
    Context 'Discovery' {
        ...
        It 'should have a valid manifest' {
            $manifest = Get-Content (Join-Path $publishPath 'windows-user.dsc.resource.json') | ConvertFrom-Json
            $manifest.type | Should -Be 'OpenDsc.Windows/User'
    ...
    Context 'Schema Validation' {
        It 'should have valid schema' {
            $exe = Join-Path $publishPath 'windows-user.exe'
            $schema = & $exe schema | ConvertFrom-Json

            $schema.'$schema' | Should -Be 'https://json-schema.org/draft/2020-12/schema'
            $schema.type | Should -Be 'object'
            $schema.properties.userName | Should -Not -BeNullOrEmpty
            $schema.required | Should -Contain 'userName'
        }
        ...
        It 'should have required properties in schema' {
            ...
            $schema.properties.userName | Should -Not -BeNullOrEmpty
            $schema.properties.userName.type | Should -Be 'string'
            $schema.properties.userName.pattern | Should -Not -BeNullOrEmpty
        }
        ...
        It 'should mark password as writeOnly in schema' {
            $exe = Join-Path $publishPath 'windows-user.exe'
            $schema = & $exe schema | ConvertFrom-Json

            $schema.properties.password | Should -Not -BeNullOrEmpty
            $schema.properties.password.writeOnly | Should -Be $true
        }
    }
